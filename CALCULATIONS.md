...
We begin with the Schwarzschild spacetime written in Schwarzschild coordinates $(t, r, \theta, \phi)$
...
Angular momentum is $L = r^2 \frac{d\phi}{d\lambda}$, so

$$\boxed{\frac{d\phi}{d\lambda} = Lr^{-2}}$$
...
$$
\frac{dv_r}{d\lambda} = \frac{b^2}{r^3} - \frac{3Mb^2}{r^4}
$$
...
$$
v_{r0} = \pm\sqrt{1 - \frac{b^2}{r_0^2}\left(1 - \frac{2M}{r_0}\right)}
$$

Where $r_0$ is the initial distance from the camera to the origin; In $v_{r0}$ the sign of the root depends on the direction of the ray: $-$ if the ray moves inward and $+$ if outward
